...
class Database:
    ...
    async def connect(
        ...
        try:
            ...
            _ = await db.create_container_if_not_exists(
            ...
            yield cls(client, db_name, history_container_name, kb_container_name)
        finally:
            await client.close()
    ...
    async def search_cache(self, query: str, openai: AsyncAzureOpenAI) -> list[dict]:
        ...
        try:
            existing_item = await container.read_item(
                item=query_hash, partition_key=query_hash
            )
    ...
    async def update_cache(
        self, query: str, result: list[dict], openai: AsyncAzureOpenAI
    ):
        container = self.client.get_database_client(self.db_name).get_container_client(
            self.cache_container_name
        )
